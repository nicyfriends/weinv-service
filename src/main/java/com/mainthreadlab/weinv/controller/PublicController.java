...
public class PublicController {
    ...
    private final String weinvContact;
    private final UserService userService;
    private final EmailService emailService;


    @Autowired
    public PublicController(UserService userService, EmailService emailService, @Value("${weinv.contact}") String weinvContact) {
        this.userService = userService;
        this.emailService = emailService;
        this.weinvContact = weinvContact;
    }


    @PostMapping("/auth/login")
    @Operation(operationId = "login", summary = "Login", tags = {"Authentication"}, responses = {
    ...
    @Operation(operationId = "contactus", summary = "Contact us", tags = {"Contact"}, responses = {
                    @ApiResponse(responseCode = "200", description = "Ok", content = @Content(mediaType = "application/json", schema = @Schema(implementation = LoginResponse.class))),
                    @ApiResponse(responseCode = "400", description = "Bad request", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class))),
                    @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class)))})
